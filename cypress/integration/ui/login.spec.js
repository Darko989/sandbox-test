describe("Login test", () => {
	...
	it("check Forget password", () => {
        ...
        cy.get('.col-xl-12 > :nth-child(3)').should("contain", "* = required field");
        cy.get('.value').should("have.css", "position", "relative");
        cy.get('.form-group > .form-text').should("contain", "Enter your email address and we will send you instructions to reset your password.");
	...
	it("requires email", () => {
        cy.get(':nth-child(1) > .value').should("have.css", "position", "relative");
	...
	it("requires password is more than 6 characters", () => {
		...
		cy.get(".invalid-feedback").contains("Password must be at least 6 characters long");
		cy.url().should("include", "/login");
	});

    //Sucessfull login and logut
